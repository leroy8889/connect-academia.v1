...
const Feed = (() => {
    ...
    function initFilters() {
        elements.filterBtns.forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                // Lire data-filter en priorité, fallback data-type
                const filterValue = btn.dataset.filter ?? btn.dataset.type ?? '';
    ...
    async function loadPosts() {
        ...
        try {
            ...
            const response = await API.get(`/api/communaute/posts?${params}`);
    ...
    async function toggleLike(postId, btn) {
        ...
        try {
            await API.post(`/api/communaute/posts/${postId}/like`);
        } catch (error) {
            // Rollback
            btn.classList.toggle('post-card__action--liked');
            if (isLiked) {
    ...
    async function bookmarkPost(postId, btn) {
        ...
        try {
            const result = await API.post(`/api/communaute/posts/${postId}/bookmark`);
